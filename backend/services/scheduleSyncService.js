...
//      0/1). The Action-3 acceptance un-deadlocks ownership resets (unclaim/
//      reclaim wipes scheduleSyncInfo while the device keeps its STEES timers):
//      without it the stored rule was misclassified as foreign forever and
//      every sync latched 'unsupported' / 'Rule2 is not free'. Timer3 (user
//      trigger) stays absolutely excluded, and plain user timers (Action 0/1)
//      stay rejected.
...
// Persist the sync outcome onto Device.scheduleSyncInfo. Terminal failures go
// through this so the stored status can never go stale: leaving a previous
// 'synced' in place after a failure made the retry sweep and startup backfill
// believe the device was converged and skip it forever (invisible self-healing
// deadlock). managedTimerIndexes is sticky ownership — preserved verbatim on
// failures (orphan cleanup depends on it), replaced only by a verified result.
...
async function runSyncDevice(deviceId, { deviceModel = Device, scheduleModel = Schedule, traceId = '?', source = 'unknown', logger = console } = {}) {
  ...
  // failure WITHOUT wiping it. Wiping here would cascade: one unexpected
  // throw erases managedTimerIndexes, which then deadlocks Rule2 recognition
  ...
  try {
    ...
    await persistOutcome(deviceModel, deviceId, managedSnapshot, 'failed', summary.error);
    return summary;
  } catch (err) {
    summary.status = 'failed';
    summary.verificationPassed = false;
